...
void HttpMultipartFormData::SetupHttpRequest(const FHttpRequestRef& HttpRequest)
{
	...
	{
		UE_LOG(LogSlackOpenAPI, Error, TEXT("Expected POST verb when using multipart form data"));
	}
...
void HttpMultipartFormData::AddFilePart(const TCHAR* Name, const HttpFileInput& File)
{
	...
	{
		UE_LOG(LogSlackOpenAPI, Error, TEXT("Failed to load file (%s)"), *File.GetFilePath());
		return;
	}
